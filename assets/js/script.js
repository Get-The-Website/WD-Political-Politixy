...
(function ($) {
  ...
  if($('#navContentmenu').length > 0){
    const navContentmenu = new bootstrap.Offcanvas('#navContentmenu');
...
// politixy_couponFun js
function politixy_couponFun(){
...
// politixy_accountCheck js
function politixy_accountCheck(accountCheckSelector){
  $(accountCheckSelector).each(function(index, item){
    $(item).on("click", function(){
      let thisSiblings = $(this).parents('.account-check').find('input');
      if(!$(thisSiblings).is(':checked')){
        $('.account-password').slideDown(200);
        console.log('tr')
      }else{
        $('.account-password').slideUp(200);
        console.log('fls')
      }
    })
  })
}
politixy_accountCheck();

let accountCheck = $('.account-check-label');
if(accountCheck){
  politixy_accountCheck(accountCheck);
}
  ...
  // =================  Sending Mail =============
  $(document).on('submit', '#contactForm,#volunteerForm, #donateForm',function(e) {
    e.preventDefault();
    var form = $(this);
